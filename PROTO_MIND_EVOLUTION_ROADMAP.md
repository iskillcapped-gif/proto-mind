...
Acceptance: Native now covers provenance/outcome/lifecycle inspection, separately confirmed candidate-to-lesson and durable-lesson-to-skill paths, and exact manual outcome capture through the existing core. Mutating skill lifecycle controls remain CLI-only; project-scoped memory, durable manual-use history and runnable skill packages remain undelivered. All rejected/stale/token-mismatch paths continue to refuse, and simply viewing the library, evidence sheet, outcome form or Workshop remains read-only.

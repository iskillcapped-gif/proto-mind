...
The 2026-09-03 [DeepSeek Harness adoption review](DEEPSEEK_HARNESS_ADOPTION_REVIEW.md) adds a second evidence source without authorizing an upstream runtime import. Session Spine v0.1, P1 synthetic Native projection, isolated P2a private store, P2b fixture transfer, P2c ordered multi-turn composition and P2d private composition evidence export are delivered. P2d rederives only explicitly selected caller-bound fixtures and writes a separately versioned run-once private bundle: ordered exact sources, exact P2a candidate, content-free parity dossier and manifest-last completion marker. Its read-only verifier repeats P1 -> P2c -> P2a and refuses content, order, schema, permission, symlink or file-set drift. It never scans, pairs or sorts a personal archive, and adds no live-store writer. Archive-wide compatibility, authenticity signatures, apply/restore lifecycle and any production integration remain separate decisions; personal history and Send/provider/UI/command authority are unchanged.

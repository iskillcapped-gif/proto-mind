...
Acceptance: Native now covers provenance/outcome/lifecycle inspection, separately confirmed candidate-to-lesson and durable-lesson-to-skill paths, exact manual outcome capture, terminal process-only outcome decisions and separately confirmed keep/durable archive through the existing core. Restore remains a separate CLI gate; revision, project-scoped memory, durable manual-use/decision history and runnable skill packages remain undelivered. All rejected/stale/token-mismatch paths continue to refuse, and simply viewing the library, evidence/decision/apply sheet, outcome form or Workshop remains read-only.

...
The 2026-09-03 [DeepSeek Harness adoption review](DEEPSEEK_HARNESS_ADOPTION_REVIEW.md) adds a second evidence source without authorizing an upstream runtime import. Session Spine v0.1, P1 synthetic Native projection, isolated P2a private store and P2b fixture transfer boundary are delivered. P2b independently reprojects one explicit canonical copied fixture, builds exact candidate bytes through the shared P2a contract, and can write only a new private evidence bundle with source/candidate hashes plus optional exact rollback preimage. Conflicts block; migration and rollback remain previews with no apply/restore/delete API. There is no default personal path or application caller, so personal history and Send/provider/UI/command authority remain unchanged. Multi-turn composition, copied-archive parity and any production integration remain separate decisions.

...
Acceptance: Native now covers provenance/outcome/lifecycle inspection, separately confirmed candidate-to-lesson and durable-lesson-to-skill paths, exact manual outcome capture and terminal process-only outcome decisions through the existing core. Actual skill lifecycle writes remain CLI-only; project-scoped memory, durable manual-use/decision history and runnable skill packages remain undelivered. All rejected/stale/token-mismatch paths continue to refuse, and simply viewing the library, evidence/decision sheet, outcome form or Workshop remains read-only.

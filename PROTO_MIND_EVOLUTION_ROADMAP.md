...
Acceptance: Native covers provenance/outcome/lifecycle inspection, separately confirmed candidate-to-lesson and durable-lesson-to-skill paths, exact manual outcome capture, process-only decisions, separately confirmed archive/restore, explicitly saved historical snapshots, scoped operator notes and manual/automatic procedure guidance. Built-ins remain separate from learned experience. Automatic project recall, lesson promotion, skill revision, pending post-restore outcome capture and executable skill packages remain undelivered. Rejected/stale/token-mismatch paths still refuse; simply viewing the library, evidence/review sheets, Workshop or starter set remains read-only.

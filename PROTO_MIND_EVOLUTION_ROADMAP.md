...
The 2026-09-03 [DeepSeek Harness adoption review](DEEPSEEK_HARNESS_ADOPTION_REVIEW.md) adds a second evidence source without authorizing an upstream runtime import. Session Spine v0.1, P1 synthetic Native projection and isolated P2a private-store foundation are delivered. P2a uses explicit-path-only canonical prepare/commit records, fsync/hash-chain/readback boundaries, one writer per session, truthful unknown recovery and a no-compaction retention preview. It has no default personal path or application caller: no personal history is read or written, and Send/provider/UI/command authority is unchanged. P2b export/migration/rollback previews and any production integration remain separate decisions.

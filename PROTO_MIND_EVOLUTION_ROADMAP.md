...
Acceptance: Native now covers provenance/outcome/lifecycle inspection and separately confirmed candidate-to-lesson and durable-lesson-to-skill paths, including receipt verification after apply and durable source inspection after restart. Mutating skill lifecycle controls and explicit manual outcome capture remain CLI-only; project-scoped memory and runnable skill packages remain undelivered. All rejected/stale/token-mismatch paths continue to refuse, and simply viewing the library, evidence sheet or Workshop remains read-only.

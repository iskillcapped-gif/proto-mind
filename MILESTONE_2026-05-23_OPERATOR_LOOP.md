...
- Command Registry v1.0 adds `/commands status|list|explain|doctor`; the current Registry contains 363 prefixes in 41 categories. It exposes category, mutation, risk, and Natural Router metadata; doctor validates duplicates, field values, natural target coverage, explicit context mutations, and exclusion of high-risk natural routes. Registry commands describe but never execute commands.
...
- v3.5c / Procedural Skill Apply Readiness adds read-only readiness, future apply plan, and Doctor views. It revalidates receipt/source/provenance/payload hashes, current Skill Library shape and SHA-256, deterministic target ID, global active/archived duplicates, and a fixed 15-field future receipt with atomic-write, verification, exact-mutation, separate-confirmation, and rollback requirements. No apply token, writer, execution, or mutation is installed.

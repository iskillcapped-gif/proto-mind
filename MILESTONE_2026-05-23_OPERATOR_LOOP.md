...
- PySide6 Cognitive Control Room v2.0.0 preserves that shared runtime and safety model while upgrading the local `.app` into an intentional cognitive console. A local/private identity bar, read-only context/capability indicators, structured operator composer, four prompt chips, and twelve grouped Control Deck actions expose Daily, Agenda, Proto, Experience, Memory Card, Skill lifecycle, warning, snapshot, and showcase views. All deck targets are Registry-confirmed `read_only=true` with `mutates=none`; no routing, cognitive behavior, store, session-log, or Context Injection semantics changed. The local launcher bundle metadata is v2.0.0.

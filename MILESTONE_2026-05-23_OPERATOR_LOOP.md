...
- Command Registry v1.0 adds `/commands status|list|explain|doctor`; the current Registry contains 361 prefixes in 41 categories. It exposes category, mutation, risk, and Natural Router metadata; doctor validates duplicates, field values, natural target coverage, explicit context mutations, and exclusion of high-risk natural routes. Registry commands describe but never execute commands.
...
- v3.4f / Learning Lifecycle Apply Readiness adds read-only lifecycle readiness, future transition plan, and Doctor reports. Current lesson provenance, exact outcome hash/signal, persistent-store SHA-256, and verified replacement state must all match; keep/reject/supersede atomic receipt and rollback contracts are visible, but the engine flag remains false and no apply command or mutation exists.

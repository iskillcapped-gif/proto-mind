...
- Command Registry v1.0 adds `/commands status|list|explain|doctor`; the current Registry contains 387 prefixes in 41 categories. It exposes category, mutation, risk, and Natural Router metadata; doctor validates duplicates, field values, natural target coverage, explicit context mutations, and exclusion of high-risk natural routes. Registry commands describe but never execute commands.
...
- v3.5e / Durable Skill Provenance Inspection embeds a hashed source/contract/payload/confirmation envelope in each newly supervised-applied skill. `/skills why <id>` verifies the chain after restart and distinguishes verified, historical, drifted, unavailable, and invalid states; `/skills provenance-doctor` audits the full library read-only. Ordinary operator skills remain valid without an invented chain, older supervised records are explicit legacy warnings, and no skill execution, migration, repair, or new writer is added.
- v3.5f / Procedural Skill Outcome Review adds read-only `/experience learning skill-outcome-review <skill_id>` and `skill-outcome-doctor`. Only exact current-process manual-use lineage tied to verified durable skill provenance is eligible; verified operator-reported success/failure/correction becomes an advisory candidate, mixed evidence remains inconclusive, and usage counters are ignored. No procedure execution, event capture, score update, store write, model/API call, or Context Injection change is added.
- v3.5g / Supervised Manual Skill Outcome Capture adds exact preview/token capture plus process-memory receipt list/inspect/Doctor views. Active session consent and current verified skill provenance are mandatory; one confirmed four-event batch records an already-performed operator action and success/failure evidence under existing bounds. Exact duplicates are run-once, receipts expire on restart, and no skill execution, persistent writer, store/session-log mutation, scoring, model/API, shell, or Context Injection change is added.
- v3.5h / Supervised Procedural Skill Outcome Decision adds exact preview/token decision plus bounded list/inspect/Doctor views. A decision requires decisive v3.5f evidence fully backed by confirmed v3.5g receipts; success permits keep, failure/mixed permits revise or archive. One terminal receipt per skill expires on restart and remains non-apply-ready. No keep/archive/revision mutation, scoring, procedure execution, writer, model/API, shell, or Context Injection change is added.
- v3.5i / Procedural Skill Lifecycle Apply Readiness adds read-only readiness, future-plan, and Doctor reports over that terminal decision. It revalidates current review/capture hashes, skill provenance/payload, complete skill record bytes, and Skill Library SHA-256. Keep remains byte-stable receipt-only, archive requires one future atomic status mutation with restore guidance, and revise requires separate versioned authoring; readiness itself generates no token, invokes no writer, and performs no execution or persistence.
- v3.5j / Supervised Procedural Skill Lifecycle Apply Pilot adds a second exact-token preview/apply gate plus receipt list/inspect/Doctor views. One current keep decision is verified without changing Skill Library bytes; one archive decision atomically changes only status/updated_at, preserves provenance and memory, and restores original bytes on failure. Revision, repeated/batch apply, procedure execution, generic dispatch, model/API, shell, and Context Injection changes are refused.
- v3.5k / Durable Procedural Skill Lifecycle Audit adds restart-safe read-only status/history/inspect/Doctor views. It verifies active supervised records, exposes drift and legacy gaps, and classifies archived records as ambiguous unless a future supported durable lifecycle envelope proves the cause; no schema or writer is added.
- v3.5l / Durable Skill Lifecycle Metadata Design Lock defines a deterministic hashed `skill.procedure.lifecycle.v1` archive envelope and exposes it through `/skills lifecycle-status --contract`. The existing Doctor validates the example and tamper refusal, but the writer remains absent and any such field in live storage is rejected until separately authorized.
- v3.5m / Durable Skill Lifecycle Writer Readiness adds `--durable` to existing lifecycle readiness/plan views. It binds current archive evidence and skill bytes to the fixed envelope blueprint, exact three-field mutation and rollback/receipt contract, while explicitly refusing to treat the current v3.5j writer as metadata-capable.
- v3.5n / Durable Skill Lifecycle Metadata Apply Pilot uses a separate exact `--durable` token to persist one archive envelope atomically, verifies only `lifecycle/status/updated_at` changed, preserves provenance and memory, rolls back exact bytes on failure, and lets restart-safe audit report `archived_verified`. Legacy archive fails closed; keep remains byte-stable and restore/revise remain separate future work.
- v3.5o / Durable Skill Lifecycle Restore Design Review adds read-only restore contract/readiness/plan/Doctor flags on existing `/skills lifecycle-*` prefixes. Only current `archived_verified` records without active duplicates qualify; the future envelope embeds the full archive evidence and locks exact mutation/receipt/rollback boundaries. No token, writer, direct restore, migration, or execution is introduced.
- v3.5p / Direct Lifecycle Status Guardrail moves the protection into `SkillLibrary.set_status`: generic archive/restore now refuses lifecycle-managed or malformed lifecycle records without touching bytes, while legacy provenanced and operator skills retain existing behavior. Restore Doctor reports the guard as installed; no writer or Registry expansion is added.
- v3.5q / Lifecycle-Managed Skill Payload Guardrail protects the remaining shared mutation paths: summary/body/tag changes and usage telemetry now refuse lifecycle-managed or malformed lifecycle records before callback, timestamp, or rewrite. Pre-lifecycle provenance and operator skills remain compatible; no command, writer, restore authorization, or execution path is added.
- v3.5r / Durable Restore Authorization Readiness reuses v3.5o validation to bind exact current hashes, immutable fields, prior archive evidence, future confirmation/run-once scope, receipt verification, and rollback into a read-only blueprint. Existing lifecycle prefixes expose the review; no token generator, authorization engine, state, writer, or mutation exists.
- v3.5s / Supervised Durable Restore Apply Pilot adds one exact current-state token and one successful restore slot per process. The existing `/skills restore` gate accepts only `<id> <token> --durable` for a current `archived_verified` skill, atomically changes only `lifecycle/status/updated_at`, embeds the full prior archive envelope, preserves immutable procedure/provenance and memory, verifies restart state `active_restored_verified`, records a fixed hashed receipt, and restores exact original bytes on any post-write failure. Generic mutation, repeated/batch restore, re-archive, revision, procedure execution, shell, model/API, and Context Injection changes remain unavailable.
- v3.5t / Durable Restore Receipt Audit adds a separately hashed restart-safe evidence contract over the embedded restore envelope and current record. Existing lifecycle prefixes list, inspect, print copyable JSON, and diagnose evidence while distinguishing an exact live process receipt from expected restart absence, legacy/incomplete receipts, orphans, and mismatch. Ten receipt fields are durably recoverable; the original 21-field process receipt is never invented or persisted. No writer, repair, re-archive/revision, procedure execution, shell, model/API, export file, or Context Injection change is added.
- v3.5u / Restored Skill Re-evaluation Design adds read-only `--post-restore` review/plan/contract/Doctor flags on existing outcome prefixes. A strict restore timestamp plus provenance, restore metadata, and receipt-evidence hashes separates new exact manual evidence from stale pre-archive or unbound later events. Legacy capture and outcome-decision paths refuse restored skills; exact new signals remain advisory and never decision/apply ready. No capture/decision writer, token, event/store mutation, procedure execution, shell, model/API, or Context Injection change is added.
- v3.5v / Supervised Post-Restore Outcome Capture Readiness adds read-only readiness/plan/contract/Doctor flags on existing capture prefixes. A canonical blueprint requires active session consent and binds exact current Skill Library/record/provenance/payload hashes, restore metadata/evidence, bounded operator evidence, the fixed four-event sequence, restore-bound call fields, and future receipt schema. Tamper or current-state drift fails closed; no token generator, writer, Experience append, receipt, lifecycle decision, procedure execution, shell, model/API, persistent mutation, or Context Injection change is added.
...
- PySide6 Cognitive Control Room v2.1.0 preserves that shared runtime and safety model while adding a dedicated contest Demo Runway beside the original Control tab. Twelve numbered controls cover readiness, system story, explicit broad-consent refusal, preview-gated exact Experience consent, one normal cognitive turn, episode/learning explanation, preview-gated exact fixed read-only runner use, evidence verification, and capture stop. Both exact controls parse only their expected fixed prefix from the immediately preceding raw preview and reject chains; they never synthesize tokens or bypass the shared handler. No cognitive, store, session-log, or Context Injection semantics changed. The local launcher bundle metadata is v2.1.0.

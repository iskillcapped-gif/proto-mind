...
- Current test count: 1086 unit tests OK.
...
v3.5m / Durable Skill Lifecycle Writer Readiness:

- Existing lifecycle readiness/plan accept `--durable` without Registry expansion and build a fixed-field archive envelope blueprint over the current v3.5h receipt plus v3.5i evidence/provenance/store revalidation.
- Four dynamic fields remain write-time-only; the plan fixes one changed record, fields `lifecycle/status/updated_at`, atomic replacement, exact-byte rollback, immutable provenance, unchanged memory, and a 21-field receipt.
- Keep is explicitly envelope-free and byte-stable; revise remains not ready. The current v3.5j writer is explicitly non-compatible and never invoked.
- Registry remains 387 commands across 41 categories. Six focused v3.5m regressions plus all prior coverage pass; the full suite is 1086 tests OK.
- No token, writer, migration, repair, procedure execution, shell, arbitrary dispatch, model/API, store/export/session-log mutation, external action, or Context Injection change was added.
...
- v3.5n / Durable Skill Lifecycle Metadata Apply Pilot: behind a new exact confirmation, upgrade only the archive path to atomically persist the v3.5l envelope with full receipt verification and exact-byte rollback; keep remains no-op and revise remains unavailable.

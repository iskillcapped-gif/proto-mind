...
- Persona Engine 0.2 keeps the strict single-Brother kernel and adds a bounded Native Inspector for an immutable hashed `PersonaSnapshot`: read-only Identity projection, no selected memory/retrieval, and current provider/model/workspace/access facts. Exact Python and Swift contracts reject widened sources or authority. The snapshot is still not wired into provider prompts; the preview performs no model/network call, command or write, exposes no facets/trait controls/private reasoning, grants no authority and leaves Context Injection unchanged.
...
- Current test count: 1540 unit tests OK (1533 previous + 7 Native Persona preview regressions).
- Native checks: `scripts/test_native.sh` passes 368 checks, adding real-bridge Persona preview rendering, layout, tamper refusal, opaque workspace identity and byte-preservation checks to the previous Full Mac Web Search/Computer Use, durable-session, PDF/history, notice, attachment-drop/layout, image, criteria/manual-review, context/artifact, work-session, hover, typography, catalog, library and grant coverage. Real stdio checks use code-only temporary fixtures without full Xcode/XCTest.
...
- Persona Engine 0.2 / Native visible preview: `persona_engine.py`, strict `persona/brother-0.1.0.json`, `native_persona.py`, `PersonaInspector.swift`, seven-case deterministic evals and `PERSONA_ENGINE_MIGRATION_MAP.md`. One Brother identity only; immutable non-authorizing snapshots, read-only Identity projection, no preview retrieval, factual current runtime and strict display validation. No production prompt activation, store writer, model/network call, Context Injection or new permission.
...
Persona Engine 0.2 / Native visible preview:

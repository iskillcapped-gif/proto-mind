...
- Current test count: 1128 unit tests OK.
...
v3.5t / Durable Restore Receipt Audit:

- Existing lifecycle prefixes now expose a read-only receipt-evidence contract, durable restore evidence list, per-skill audit, copyable JSON view, and Doctor without Registry expansion.
- A verified current restore envelope deterministically reconstructs and hashes only ten receipt fields that survived restart; eleven process-only fields remain explicitly unavailable.
- A live process receipt is compared against the durable envelope/current record when present. Missing receipts after restart are expected; incomplete, orphan, duplicate, hash-mismatched, or state-mismatched receipts are surfaced without repair.
- The JSON view is printed only and never written. The original 21-field apply receipt is never reconstructed, persisted, or claimed as durable history.
- Registry remains 387 commands across 41 categories; the full suite is 1128 tests OK. No writer, re-archive/revision capability, procedure execution, model/API, store/export/session-log, or Context Injection change was added.
...
- v3.5u / Restored Skill Re-evaluation Design: require new provenance-bound manual-use evidence after restore before any future keep/re-archive/revision decision; design only before another writer.

...
- Persona Engine 0.2 keeps the strict single-Brother kernel and adds a bounded Native Inspector for an immutable hashed `PersonaSnapshot`, plus provider-parity/provenance/readiness evidence for Codex, Ollama and the Mock control. Exact Python and Swift contracts reject widened sources, identity drift, replaceable safety layers or authority. The snapshot is still not wired into provider prompts; preview/readiness perform no model/network call, retrieval, command, activation or write, expose no facets/trait controls/private reasoning, grant no authority and require Context Injection disabled before future activation.
...
- Current test count: 1552 unit tests OK (1540 previous + 12 Persona provider-readiness regressions).
- Native checks: `scripts/test_native.sh` passes 374 checks, adding real-bridge adapter parity, Mock control-only, nine-gate/no-side-effect evidence, path exclusion and activation/safety tamper refusal to the previous Persona preview, Full Mac Web Search/Computer Use, durable-session, PDF/history, notice, attachment-drop/layout, image, criteria/manual-review, context/artifact, work-session, hover, typography, catalog, library and grant coverage. Real stdio checks use code-only temporary fixtures without full Xcode/XCTest.
...
Persona Engine 0.2 Provider Readiness / Native 0.18.0:

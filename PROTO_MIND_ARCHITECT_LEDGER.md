...
- Current test count: 1564 unit tests OK (1552 previous + 12 controlled-activation regressions).
- Native checks: `scripts/test_native.sh` passes 387 checks, adding preferences-v1 fail-closed migration, two-phase opt-in, stable readiness matching, private-preference-only activation, exact rollback and bounded receipt checks to the previous Persona readiness, Full Mac Web Search/Computer Use, durable-session, PDF/history, notice, attachment-drop/layout, image, criteria/manual-review, context/artifact, work-session, hover, typography, catalog, library and grant coverage. Real stdio checks use code-only temporary fixtures without full Xcode/XCTest.
...
Persona Engine 0.3 Controlled Activation / Native 0.19.0:

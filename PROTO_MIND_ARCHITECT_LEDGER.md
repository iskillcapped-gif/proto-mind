...
- Current test count: 1569 unit tests OK (1564 previous + 5 EV-04/rollback regressions).
- Native checks: `scripts/test_native.sh` passes 390 checks, adding legacy provenance honesty, empty no-session Memory Workshop, auto-promotion refusal and scope-boundary rendering to the previous Persona readiness, Full Mac Web Search/Computer Use, durable-session, PDF/history, notice, attachment-drop/layout, image, criteria/manual-review, context/artifact, work-session, hover, typography, catalog, library and grant coverage. Real stdio checks use code-only temporary fixtures without full Xcode/XCTest.
...
EV-04 Cognitive Memory Loop v1 / Persona 0.3.1 / Native 0.20.0:
...
- Persona 0.3 / Controlled Activation: add one explicit local opt-in for the validated Brother snapshot, route it through the already verified Codex/Ollama placements, keep Mock control-only, preserve provider safety layers and existing retrieval/model-call counts, and provide an immediate visible rollback to the byte-for-byte legacy prompt path. Context Injection must stay independently disabled and activation must never grant authority or write core stores.

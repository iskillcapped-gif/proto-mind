...
- Current test count: 1517 unit tests OK (1510 previous + 7 local-capability/lifecycle/version regressions).
...
- Persona 0.1 / Model-independent Persona Foundation: implement the accepted [single-personality plan](PERSONA_ENGINE_PLAN.md) only after checkpointing Native 0.16.0. Reuse Identity, provenance-memory and permission contracts; add a bounded read-only `PersonaSnapshot` and invariant evals without user-facing facets, selectable modes, production prompt changes, Context Injection, store migration or new authority.

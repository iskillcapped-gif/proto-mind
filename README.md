...
The [DeepSeek Harness adoption review](DEEPSEEK_HARNESS_ADOPTION_REVIEW.md) compares the official MIT-licensed Developer Preview with Proto-Mind at an exact upstream commit. Proto-Mind does not embed that runtime. The extracted pure-Python Session Spine includes exact P1 Native projection, an isolated P2a private-store foundation, the P2b single-fixture transfer boundary, P2c ordered multi-turn composition, P2d private multi-turn evidence export and P2e archive-copy compatibility audit. P2e accepts only immutable bytes for one explicitly supplied `conversations.json` copy plus a sorted exact SHA-256 manifest of copied work-session records. It accepts bounded Native history v1-v5, validates the structural and lineage fields it inspects, reuses the current work-session/Turn Lineage/P1 validators and emits a deterministic content-free `OK/WARN/ERROR` report for compatible, legacy, missing, invalid, duplicate and orphaned evidence. It neither opens a path nor writes a report, guesses a run, calls a model/command/tool, migrates data or grants writer authority. The manifest proves the supplied bytes, not that the caller copied every record from an unseen source. P2d exact-content bundles remain private and not publication-safe. There is no default personal path or production caller, so personal history, Send, providers, UI, commands and permissions remain unchanged.

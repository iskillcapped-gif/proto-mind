...
The [DeepSeek Harness adoption review](DEEPSEEK_HARNESS_ADOPTION_REVIEW.md) compares the official MIT-licensed Developer Preview with Proto-Mind at an exact upstream commit. Proto-Mind does not embed that runtime. The extracted pure-Python Session Spine includes exact P1 Native projection, an isolated P2a private-store foundation, the P2b single-fixture transfer boundary and P2c ordered multi-turn composition. P2c accepts only immutable caller-supplied fixtures bound to an exact SHA-256 order manifest and expected conversation, revalidates each through P1, rebases sequence provenance without changing canonical payloads, and proves one in-memory P2a candidate by exact replay. It does not scan or sort personal history, write an export/store, infer pairing or grant authority. P2b's exact-content bundles remain private evidence, not publication-safe reports. There is no default personal path, apply/restore writer or production caller, so personal history, Send, providers, UI, commands and permissions remain unchanged.

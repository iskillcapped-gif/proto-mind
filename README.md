...
Post-contest personal development now targets a real SwiftUI/AppKit application with a Codex-inspired conversation workspace, while preserving the existing Python cognitive core and operator commands. The current native workspace is **Native 0.31.0 / Automatic Skill Guidance**, extending mode-bound Codex continuity, Full Mac Computer Use, durable sessions, selected PDF/image/text inputs, live Web Search, attachment recovery/drop, run notices, the Context And Artifact Desk, manual acceptance, EV-01 reliable work sessions, an inspectable Brother self-model and explicit Native candidate-to-lesson-to-skill review; it does not replace the submitted contest build.

**Automatic skills (0.31.0):** write a normal task and Send. **Skills / Auto** in the Codex composer lets the selected subscription model choose zero to two relevant active, current-provenance verified procedures. It defaults on per conversation; turn it off or prepare a manual selection to override it. No goal/criteria form is required. With an eligible catalog, one separate tool-free, ephemeral selector turn uses low effort when supported (otherwise the model default); the main task keeps your chosen effort, saved Codex session and existing permissions. The chat and private journal show the choice, source-version hashes and suggested checks, not a fabricated success receipt. Empty/unsupported libraries skip selection without initializing or promoting records. Invalid selection, Stop or source drift never auto-retries. This is guidance, not automatic skill creation, learning, new permission or a procedure interpreter. The current personal baseline has two excluded legacy records and zero eligible procedures; ordinary tasks still work without skill guidance. [Flow, privacy and evaluation](NATIVE_MACOS_ROADMAP.md#ev-04-automatic-skill-guidance--native-0310).

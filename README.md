...
**Session Spine Durable Intent Preparation (P2i, dormant):** Native now has a private, write-once installation identity contract and an exact throwable `ChatStore.saveAndReadBack` boundary, while Python has an explicit-path immutable prepared/committed intent store over the P2h handshake. A prepared intent survives relaunch; a first apply can record the exact P2h receipt, a lost apply response can add only the missing commit marker after a verified no-write replay, and a closed intent is run-once. Malformed, symlinked, unexpected or conflicting evidence blocks without cleanup. These primitives are deliberately not called by `AppModel`, the bridge or UI, so normal Send, history and personal state behave exactly as before. No default Spine/intent path, live writer, migration, retry, repair, backfill, model/tool call or permission was added. [Contract and acceptance](NATIVE_MACOS_ROADMAP.md#session-spine-durable-intent-preparation--p2i).

...
Post-contest personal development now targets a real SwiftUI/AppKit application with a Codex-inspired conversation workspace, while preserving the existing Python cognitive core and operator commands. The current native workspace is **Native 0.18.0 / Persona Provider Readiness**, extending mode-bound Codex continuity, Full Mac Computer Use, durable sessions, selected PDF/image/text inputs, live Web Search, attachment recovery/drop, run notices, the Context And Artifact Desk, manual acceptance, EV-01 reliable work sessions and an inspectable Brother self-model with provider-parity gates; it does not replace the submitted contest build.
...
**Persona Provider Readiness (0.18.0):** **Обзор ядра > Persona Inspector** still compiles the checked-in Brother kernel, a private read-only Identity projection and current factual runtime into one hashed `PersonaSnapshot`. It now also derives three non-executing adapter projections: Codex `baseInstructions` refreshed on thread start/resume, Ollama's per-request system message, and a Mock control that is explicitly not activatable. Exact provenance receipts bind kernel, Identity, task, runtime and every already-selected memory reference; one provider-independent invariant hash must survive while runtime hashes remain honestly different. Nine visible gates require parity, bounded prompts, separate provider safety instructions, no added authority/side effects and disabled Context Injection. The request/token/path boundaries remain closed and the deterministic readiness eval passes for Codex/Ollama/Mock. This milestone is **inspection only**: no adapter consumes the context, no toggle exists, Mock cannot impersonate a production adapter, and no model/network call, retrieval, command, permission or store write occurs. Controlled activation and rollback remain Persona 0.3.
...
**Native library:** expand the sidebar's Library and Core group, then open Memory, Goals, or Skills for literal text search, current/history/all filters, and source-backed detail cards. Native 0.16.0 routes those reads through exact typed local `search`/`fetch` contracts and validates their structured result envelopes before rendering. These views read the existing Proto-Mind core stores, not the conversation's bound source folder. They do not call a model, run commands, update usage counts, initialize missing stores, save navigation state, or attach records to a prompt. Source errors, omitted records, truncation, and changed-since-list details are visible; stored provenance is not presented as freshly verified truth. Refresh is manual, and editing/applying/running records is not part of this view.

**Checkpoint coverage:** `/memory backup` now includes native source/tests, scripts, root Markdown documentation and the existing Python/data/export package, while excluding build caches. Completed archives are private and never overwrite a same-second checkpoint. Native conversations/preferences/work-session evidence and `codex_threads.json` outside the project still require a separate private backup. The binding file alone does not back up Codex rollouts; credentials and provider history remain managed in the private Codex profile. Never copy that profile into source or publish private archives.
...
The delivered [Persona Engine foundation and readiness layers](PERSONA_ENGINE_PLAN.md) define Brother as one continuous model-independent personality. A strict checked-in kernel, immutable hashed snapshots, a read-only Identity projection, source-linked selected-memory references, an explicit non-authorizing self-model and provider-parity/provenance gates live in `proto_mind/persona_engine.py` and `proto_mind/persona_activation_readiness.py`. They deliberately exclude user-selectable facets, personality modes and trait sliders: adaptation follows the current task, evidence and risk, while existing identity, memory provenance and permission systems remain the sources of truth. Persona remains outside the production prompt path until the separately gated Persona 0.3 milestone.

...
The [DeepSeek Harness adoption review](DEEPSEEK_HARNESS_ADOPTION_REVIEW.md) compares the official MIT-licensed Developer Preview with Proto-Mind at an exact upstream commit. Proto-Mind does not embed that runtime. The first extracted idea is an isolated pure-Python append-only Session Spine contract: source-linked surface replacement and fail-closed replay, with no persistence, provider, UI, command, or permission integration yet.

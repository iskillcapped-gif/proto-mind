...
v3.5i adds read-only `/experience learning skill-outcome-lifecycle-readiness|skill-outcome-lifecycle-plan <skill_id|decision_receipt_id>` and `skill-outcome-lifecycle-doctor`. The reviewer revalidates the exact terminal decision against current v3.5f evidence, every v3.5g capture receipt, durable skill provenance, the complete current skill record hash, and current Skill Library SHA-256. Future `keep` is constrained to a receipt-only byte-stable acknowledgement, `archive` to one atomic `active → archived` status change with `/skills restore` rollback, and `revise` to a separately authored versioned replacement while the original remains active. Readiness itself generates no token, invokes no lifecycle writer, runs no procedure, and mutates no store or process receipt.

v3.5j adds a separately registered confirmation-required lifecycle writer for only those current `keep` and `archive` decisions. `/experience learning skill-outcome-lifecycle-apply-preview <id>` emits a token bound to the exact decision, current evidence/captures, complete skill record hash, and Skill Library SHA-256; `/experience learning apply skill-outcome-lifecycle <id> <token>` consumes the single process slot. Keep is verified as a byte-identical no-op. Archive atomically changes exactly one record and only `status` plus `updated_at`, verifies immutable provenance and unchanged persistent memory, and restores exact original bytes on any failure. Receipts remain bounded process memory and expose `/skills restore <id>` guidance. Revision, second apply, procedure execution, batch/generic dispatch, shell, model/API, and Context Injection changes remain unavailable.

...
Post-contest personal development now targets a real SwiftUI/AppKit application with a Codex-inspired conversation workspace, while preserving the existing Python cognitive core and operator commands. The current native workspace is **Native 0.43.0 / Session Spine Single-Turn Forward Writer P2l**, extending source-grounded memory suggestions, automatic project recall, mode-bound Codex continuity, Full Mac Computer Use, durable sessions, selected PDF/image/text inputs, live Web Search, attachment recovery/drop, run notices, the Context And Artifact Desk, manual acceptance, EV-01 reliable work sessions, an inspectable Brother self-model and explicit Native candidate-to-lesson-to-skill review; it does not replace the submitted contest build.

**Session Spine Single-Turn Forward Writer (P2l / Native 0.43.0):** an exact-linked answer can reach a real writer only through fresh Live Preview, per-launch P2j arming, separate P2k rehearsal acceptance and a final self-hashed P2l preview with an exact `CONFIRM-SESSION-SPINE-...` phrase. Immediately before the writer, Native saves and reads back current history, creates or verifies one private installation identity, and the bridge revalidates history, Work Session, Turn Lineage, fixed paths, source bytes and gate hashes. P2i then prepares one immutable intent, P2g commits one CAS event and P2i closes the intent with a verifiable receipt. Wrong/stale input writes no Spine state; a changed history readback clears one-time grants; crash-window evidence can only resume an exact prepared intent; a closed turn has no second token. The path never calls a model, provider, slash command or tool, changes no permissions or Context Injection, performs no legacy backfill/repair/delete, and cannot ingest another turn automatically. Full write acceptance is exercised only against disposable synthetic state; personal activation remains a separate explicit UI action. [Contract and acceptance](NATIVE_MACOS_ROADMAP.md#session-spine-single-turn-forward-writer--p2l--native-0430).

**Session Spine Personal Acceptance Rehearsal (P2k / Native 0.42.0):** an exact `ARMED` P2j candidate can now open one bounded local rehearsal showing the current private state root, the installation-identity path, the two still-uninitialized future store paths and a fixed five-case crash/recovery matrix. Every path and source is rechecked and bound into a content-free SHA-256 candidate. Explicit acceptance lives only in process memory and can be revoked; relaunch, source drift, a different state root or unexpected store evidence removes or blocks it. `ACCEPTED` means only that the operator reviewed this exact future design. It does not create an identity, intent, Session Spine, history write, provider call, command, permission or writer authority, and it does not authorize the next milestone. Existing evidence requires manual inspection without retry or cleanup. [Contract and acceptance](NATIVE_MACOS_ROADMAP.md#session-spine-personal-acceptance-rehearsal--p2k--native-0420).

**Paged Transcript Rendering (0.41.0):** Native keeps the complete conversation in its existing in-memory and private-history model, but initially constructs only the latest 80 message views. An explicit top-of-history control reveals earlier messages in 60-message pages and anchors the viewport to the previously visible first message. Incoming replies cannot evict the earliest rendered row while the operator is reading older history; following the latest output keeps the bounded window. This adds no history migration, truncation, background loading, provider call, permission or Context Injection change. A 1,000-message synthetic layout regression and a live 240-message paging check cover the boundary. Repeated manual expansion can still eventually render the whole conversation; AppKit virtualization remains a measured fallback rather than a premature rewrite. [Contract and acceptance](NATIVE_MACOS_ROADMAP.md#paged-transcript-rendering--native-0410).

**Transcript Scroll Stability (0.40.1):** a macOS CPU resource report identified a SwiftUI AttributeGraph loop after a long-lived window resumed and its chat was scrolled. Selectable variable-height lazy rows and animated hover updates participated in repeated layout invalidation. Chat rows now use a stable non-lazy stack, text selection is attached only to leaf text, hover highlighting is immediate rather than animation-queued, and bottom-follow state ignores unchanged values. The fix changes no chat data, provider behavior, command, permission or Context Injection state. A 96-message layout regression and a live repeated-scroll soak cover the failure boundary; Native 0.41.0 adds bounded paging for larger histories. [Incident evidence and acceptance](NATIVE_MACOS_ROADMAP.md#transcript-scroll-stability--native-0401).

**Session Spine Activation Readiness (P2j / Native 0.40.0):** the existing exact-linked Live Preview can now open a second, content-free readiness sheet for that same verified turn. It reads an already-existing installation identity if present, treats absence as clean uninitialized state, and exposes `INACTIVE`, `ARMED` or `RECOVERY_REQUIRED` without creating, repairing or deleting anything. An acknowledged button can arm one candidate only in process memory; the grant is bound to conversation/run/preview/identity evidence, resets on relaunch or context change and still reports `writer_active=false`. The Settings screen makes this boundary visible. No default Spine/intent path, installation identity creation, intent prepare/apply, history readback write, provider/model/command/tool call, migration, legacy backfill, permission or Context Injection change was added. P2k now supplies the still-non-authorizing separate acceptance rehearsal; neither milestone activates a writer. [Contract and limits](NATIVE_MACOS_ROADMAP.md#session-spine-activation-readiness--p2j--native-0400).

**Session Spine Durable Intent Preparation (P2i foundation):** Native has a private, write-once installation identity contract and an exact throwable `ChatStore.saveAndReadBack` boundary, while Python has an explicit-path immutable prepared/committed intent store over the P2h handshake. A prepared intent survives relaunch; a first apply can record the exact P2h receipt, a lost apply response can add only the missing commit marker after a verified no-write replay, and a closed intent is run-once. Malformed, symlinked, unexpected or conflicting evidence blocks without cleanup. P2l now consumes these primitives only behind its exact single-turn gates and fixed private paths; normal Send still does not invoke them. No migration, automatic retry/repair/backfill, model/tool call or permission was added. [Contract and acceptance](NATIVE_MACOS_ROADMAP.md#session-spine-durable-intent-preparation--p2i).

**Session Spine Commit/Recovery Handshake (P2h, detached):** a stable owner identity now derives from the exact Native bundle ID plus an explicit installation UUID, never a PID, account name or permission. One content-free self-hashed handshake binds an already-saved and read-back exact message pair, canonical Work Session, immutable Turn Lineage receipt and P2g CAS candidate. Its recovery inspector distinguishes READY, COMMITTED, source drift, orphaned completed runs, store-only conflicts and an `UNKNOWN` Spine tail. The gated apply path can write only the explicitly supplied detached Spine store; history and Work Session remain read-only. History must be saved before Spine, because a completed run retains only a bounded response preview and cannot reconstruct a lost full answer. Lost apply responses replay as no-write; later operator review is accepted only after an exact commit, through the stable turn receipt, and otherwise requires a fresh handshake. This is an isolated design proof with no Native bridge/UI caller, default path, durable handshake journal, legacy backfill, automatic repair or production activation. [Contract and crash-window matrix](NATIVE_MACOS_ROADMAP.md#session-spine-commitrecovery-handshake--p2h).
...
The [DeepSeek Harness adoption review](DEEPSEEK_HARNESS_ADOPTION_REVIEW.md) compares the official MIT-licensed Developer Preview with Proto-Mind at an exact upstream commit. Proto-Mind does not embed that runtime. The extracted Session Spine now spans exact P1 Native projection, P2a-P2f detached storage/transfer/composition/audit evidence, the P2g forward writer, the P2h commit/recovery handshake and dormant P2i durability preparation. P2i adds a private Native installation identity, exact throwable history readback and an explicit immutable intent lifecycle that closes lost-response recovery without a duplicate Spine write. It still does not activate a production writer: no `AppModel`/bridge/UI caller or default personal path exists, legacy turns are never inferred or backfilled, process death can still leave an `UNKNOWN` Spine tail for manual recovery, and chat/work-session/spine persistence is not one cross-store transaction. P2d exact-content bundles remain private and not publication-safe.
...
Current development verification baseline: Python 3.11, 2095 unit tests plus 927 native checks and 7/7 + 7/7 + 8/8 Persona foundation/readiness/runtime evals, 387 registered command prefixes across 41 categories, Context Injection disabled, no production Session Spine writer, and a four-command fixed read-only core runner allowlist. The separately granted Native Full Mac + Internet + Computer Use adapter is not that core runner. Post-contest native work is separate from the submitted provenance baseline.

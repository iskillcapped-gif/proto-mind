...
The [DeepSeek Harness adoption review](DEEPSEEK_HARNESS_ADOPTION_REVIEW.md) compares the official MIT-licensed Developer Preview with Proto-Mind at an exact upstream commit. Proto-Mind does not embed that runtime. The extracted pure-Python Session Spine includes exact P1 Native projection, an isolated P2a private-store foundation, the P2b single-fixture transfer boundary, P2c ordered multi-turn composition, P2d private multi-turn evidence export, P2e archive-copy compatibility and P2f explicit private-backup acceptance. P2f opens only one operator-authorized absolute `.tar.gz` path whose bytes must match an independently supplied SHA-256. It validates a closed credential-path-free member allowlist, ignores bounded known macOS AppleDouble/provenance metadata, extracts nothing to disk and passes only exact history/work-session bytes to P2e. Its deterministic content-free report exposes legacy/compatible/invalid counts and file-mode warnings without names, paths or source text. The accepted private backup contains no invalid or incompatible evidence but is entirely pre-lineage legacy, so no links may be backfilled by guesswork; future Session Spine authority must be forward-only unless a separate migration contract is justified. P2d exact-content bundles remain private and not publication-safe. No production caller, writer, model/tool call, permission change, migration or repair was added.

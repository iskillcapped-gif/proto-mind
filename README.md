...
The [DeepSeek Harness adoption review](DEEPSEEK_HARNESS_ADOPTION_REVIEW.md) compares the official MIT-licensed Developer Preview with Proto-Mind at an exact upstream commit. Proto-Mind does not embed that runtime. The extracted pure-Python Session Spine includes exact P1 Native projection, an isolated P2a private-store foundation and a P2b fixture-transfer boundary. P2b revalidates one explicit copied fixture, builds exact P2a candidate bytes and can write only a new private export bundle with a hashed manifest and optional byte-exact rollback preimage. These exact-content bundles are private evidence, not publication-safe reports. Migration, rollback and conflicts remain no-write previews; there is no default personal path, archive scan, apply/restore writer or production caller, so personal history, Send, providers, UI, commands and permissions remain unchanged.

...
The [DeepSeek Harness adoption review](DEEPSEEK_HARNESS_ADOPTION_REVIEW.md) compares the official MIT-licensed Developer Preview with Proto-Mind at an exact upstream commit. Proto-Mind does not embed that runtime. The extracted pure-Python Session Spine includes exact P1 Native projection, an isolated P2a private-store foundation, the P2b single-fixture transfer boundary, P2c ordered multi-turn composition and P2d private multi-turn evidence export. P2d accepts only an already validated composition over immutable caller-supplied fixtures, rederives it before writing, and creates a new private run-once bundle with ordered exact sources, the exact P2a candidate, a content-free parity dossier and a self-hashed manifest written last. Its read-only verifier independently repeats P1 -> P2c -> P2a and requires byte-exact source, dossier and candidate parity. It does not scan or sort personal history, infer pairing, apply/restore/delete/compact a store or grant authority. Exact-content bundles are private evidence, not publication-safe reports; their self-hash proves consistency, not external authenticity. There is no default personal path or production caller, so personal history, Send, providers, UI, commands and permissions remain unchanged.

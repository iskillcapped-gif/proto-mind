...
Post-contest personal development now targets a real SwiftUI/AppKit application with a Codex-inspired conversation workspace, while preserving the existing Python cognitive core and operator commands. The current native workspace is **Native 0.17.0 / Persona Snapshot Inspector**, extending mode-bound Codex continuity, Full Mac Computer Use, durable sessions, selected PDF/image/text inputs, live Web Search, attachment recovery/drop, run notices, the Context And Artifact Desk, manual acceptance, EV-01 reliable work sessions and an inspectable Brother self-model; it does not replace the submitted contest build.
...
**Persona Snapshot Inspector (0.17.0):** **Обзор ядра > Persona Inspector** compiles the checked-in Brother kernel, a private read-only Identity projection and the current factual provider/model/workspace/access controls into one hashed `PersonaSnapshot`. The bridge accepts an exact request shape and verifies Full Mac only with the current conversation/workspace token; Swift then validates the envelope, single-persona invariant, source bounds, memory absence and runtime coherence before rendering. Workspace identity is opaque, the operator name and permission token are excluded, and no memory retrieval, model/network call, command, store write or Context Injection change occurs. This is visible Persona 0.2 only: the snapshot is explicitly **not active in Codex, Ollama or Mock prompts** and grants no authority. Provider activation and rollback remain a later gated milestone.

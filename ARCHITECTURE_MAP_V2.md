...
`proto_mind/session_spine.py` + `proto_mind/native_session_spine.py` + `proto_mind/session_spine_store.py` + `proto_mind/session_spine_transfer.py` + `proto_mind/session_spine_composition.py` + `proto_mind/session_spine_composition_transfer.py` + `proto_mind/session_spine_archive_copy.py` + `proto_mind/session_spine_private_backup.py` + `proto_mind/session_spine_forward.py` + `proto_mind/session_spine_handshake.py` + `proto_mind/session_spine_intent.py` + `native/Sources/SessionSpineDurability.swift` + `native/Sources/SessionSpineReadiness.swift` (DeepSeek Harness extraction / Session Spine v0.1, Native projection P1, private store P2a, fixture transfer P2b, ordered composition P2c, private composition evidence P2d, archive-copy compatibility P2e, explicit private-backup acceptance P2f, isolated forward writer/read P2g, detached commit/recovery handshake P2h, dormant durability preparation P2i, explicit read-only activation readiness P2j)
...
- P2i makes the remaining durability primitives concrete without installing them in production. Swift can create/load one canonical private installation identity and can save then exactly read back throwable history bytes. Python persists a full content-free P2h handshake as an immutable prepared record and the strictly verified P2h apply receipt as an immutable committed marker. Explicit relaunch inspection distinguishes ready apply, marker-only recovery and closed replay; unexpected files, symlinks, malformed evidence, changed sources and scope conflicts fail closed with no cleanup. No Native runtime caller invokes these APIs.
- P2j places a production-visible but non-authorizing readiness layer over one already validated Live Preview. Swift revalidates the current exact message/reference/run evidence, performs only `load()` on the optional installation identity and emits a content-free hashed report. Explicit acknowledgement arms one candidate in memory until relaunch or context drift; recovery evidence blocks without cleanup. The gate never calls `loadOrCreate`, history readback write, Python intent/apply or a default Spine path, and always reports the writer inactive.
- `proto_mind/tests/test_session_spine.py`, `proto_mind/tests/test_native_session_spine.py`, `proto_mind/tests/test_session_spine_store.py`, `proto_mind/tests/test_session_spine_transfer.py`, `proto_mind/tests/test_session_spine_composition.py`, `proto_mind/tests/test_session_spine_composition_transfer.py`, `proto_mind/tests/test_session_spine_archive_copy.py`, `proto_mind/tests/test_session_spine_private_backup.py`, `proto_mind/tests/test_session_spine_forward.py`, `proto_mind/tests/test_session_spine_handshake.py` and `proto_mind/tests/test_session_spine_intent.py` prove the detached contracts. Writes use only disposable explicit test directories. Swift durability/readiness checks also use disposable state. There is no default Spine/intent location, inferred pairing/backfill, production writer/apply command, personal-history migration, automatic repair, delete/compaction, provider/model call or permission integration. The spine is not an authoritative history source. See `DEEPSEEK_HARNESS_ADOPTION_REVIEW.md` for the comparison, license boundary, priorities and explicit non-adoptions.

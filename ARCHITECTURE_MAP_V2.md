...
`proto_mind/persona_engine.py` + `proto_mind/persona/brother-0.1.0.json` + `proto_mind/persona_evals.py` + `proto_mind/persona_activation_readiness.py` + `proto_mind/persona_activation_evals.py` + `proto_mind/native_persona.py` + `native/Sources/PersonaInspector.swift` (Persona Engine 0.2 readiness / Native 0.18.0)
...
- Native 0.18.0 adds exact read-only `persona_readiness` evidence without changing the reasoner path. A future prompt projection is rendered and hashed in memory for Codex `baseInstructions`, Ollama's system message and the Mock control adapter; Mock is never activation-capable. Kernel, Identity, task and already-selected memory produce one invariant hash, while provider/runtime facts retain separate hashes. Provenance identifies every bounded source and memory reference. Nine fail-closed gates require provider parity, prompt bounds, immutable provider safety layers, no added authority/side effects and disabled Context Injection. Swift validates and displays only the summary. No projection is dispatched to a model and no activation preference exists yet.

`native/` + `proto_mind/native_bridge.py` + `proto_mind/native_codex.py` + `proto_mind/native_computer_use.py` + `proto_mind/native_codex_threads.py` + `proto_mind/native_agent.py` + `proto_mind/native_agent_contract.py` + `proto_mind/native_agent_evals.py` + `proto_mind/native_progress.py` + `proto_mind/native_workspace.py` + `proto_mind/native_library.py` + `proto_mind/local_knowledge_capabilities.py` + `proto_mind/native_work_sessions.py` + `proto_mind/native_desk.py` + `proto_mind/native_review.py` + `proto_mind/native_images.py` + `proto_mind/native_pdf.py` + `proto_mind/native_persona.py` + `proto_mind/persona_activation_readiness.py` (Native 0.18.0 / Persona Provider Readiness)

- Native 0.18.0 is a real SwiftUI/AppKit executable, built as `dist/Proto-Mind Native.app`. Its Codex-inspired workspace includes mode-bound durable conversations, bounded attachments/context/evidence, a private work journal, separately granted Full Mac shell/Web Search and signed OpenAI Computer Use, plus a read-only PersonaSnapshot inspector with provider readiness evidence. The old PySide bundle remains a fallback; the Python cognitive core and production prompt path are not rewritten.
...
- `native_codex_threads.py` owns bounded private `codex_threads.json` v2 bindings keyed by Native conversation plus instruction mode (`chat` or `full_access`) and exact optional workspace identity. First use of each mode records a non-ephemeral `thread/start`; later turns resume only that mode's thread and revalidate ID, cwd, sandbox, approval policy and instruction sources. Bounded local history is quoted only on first creation per mode. Legacy v1 rows remain readable as historical identifiers but are never assigned to a mode or resumed automatically. Corruption, resume failure, policy drift or workspace mismatch fails closed without fallback or repair. Explicit reset removes all mode bindings for one conversation and revokes live agent grants; old Native history/provider rollouts remain. The provider profile uses local `save-all`; it can contain private prompts, replies and tool output and has no automatic pruning UI.
...
- `native_desk.py` and `ContextArtifactDesk.swift` add Native-only read-only `context_preview`, `artifact_list` and `artifact_preview` RPCs. Pre-send inspection reuses actual file/hash/history bounds, reports cloud/local destination and shared-core versus workspace scope, and does not run recall, a command or a provider. Selected sources are not silently refreshed. The full future system prompt is not claimed to be previewed.

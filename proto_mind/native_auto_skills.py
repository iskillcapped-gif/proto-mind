...
SCHEMA = "proto_mind.native_auto_skills.v1"
MAX_CATALOG = 32
MAX_SELECTED = 2
MAX_CHECKS = 4
...
def validate_auto_skills(value: dict, record: dict | None = None) -> None:
    ...
    if (value["schema"] != SCHEMA or value["state"] not in STATES
            ...
            or any(type(value[key]) is not int or not 0 <= value[key] <= 5000
    ...
    if (len(set(ids)) != len(ids) or bool(selected) != (value["state"] == "selected")
...
class AutoSkills:
    def __init__(self, root, *, conversation: str, workspace: dict | None, text: str, mode: str):
        ...
        for record in sorted(records, key=lambda row: row["id"]):
            try:
                ...
                reference = {"skill_id": record["id"], "skill_name": contract["name"], "skill_record_hash": digest(record),
        ...
        self.report["eligible_count"] = len(self.rows)
        # Stable bounded catalog, not a hidden keyword/LLM prefilter. Omission is explicit.
        self.rows = dict(list(self.rows.items())[:MAX_CATALOG])
        self.catalog = [{"skill_id": key, "name": row["contract"]["name"][:200],
    ...
    def guidance(self) -> str:
        ...
        return ("Automatically selected procedure guidance for THIS turn follows as quoted data. "
                ...
                "Source lineage was checked, not effectiveness or current task success. "

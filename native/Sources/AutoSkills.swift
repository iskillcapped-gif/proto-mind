...
struct NativeAutoSkillsReport: Equatable {
    ...
    init(_ value: JSONValue, run: JSONValue? = nil) throws {
        let fields: Set<String> = ["schema", "conversation_id", "workspace", "goal_sha256", "access_mode", "state",
        ...
        guard case .object(let raw) = value, Set(raw.keys) == fields,
              value["schema"] == .string("proto_mind.native_auto_skills.v1"), UUID(uuidString: value["conversation_id"].text) != nil,
              ...
              case .array(let selected) = value["selected"], selected.count <= 2,
        ...
        for row in selected {
            guard case .object(let reference) = row,
                  Set(reference.keys) == ["skill_id", "skill_name", "skill_record_hash", "source_lesson_id", "provenance_hash", "contract_hash", "lifecycle_state"],
    ...
    private static func count(_ value: JSONValue) -> Bool {
        if case .number(let number) = value { return number.isFinite && number.rounded() == number && (0...5000).contains(number) }
        return false
    }
...
struct AutoSkillsMenu: View {
    ...
    var body: some View {
        Menu {
            ...
            Text("Один короткий запрос Codex при наличии проверенных навыков")
            ...
            Divider()
            Button("Посмотреть навыки…") { Task { await model.showLibrary(.skills) } }
        } label: {
        ...
        DisclosureGroup {
            VStack(alignment: .leading, spacing: 8) {
                ...
                ForEach(report.selected, id: \.["skill_id"].text) { row in
                    Text("\(row["skill_name"].text) · \(row["skill_id"].text) · версия \(row["contract_hash"].text.prefix(12))").textSelection(.enabled)
                }
                ...
                Text("Каталог: \(report.value["catalog_count"].integer) · исключено: \(report.value["excluded_count"].integer). Общая библиотека, не отдельная память проекта.")
                if report.value["catalog_truncated"].flag { Text("Показаны первые 32 подходящие по статусу записи; остальные не участвовали в отборе.").foregroundStyle(.orange) }
